...
import yfinance as yf #replace mpf
from app.HarmonicPatterns.settings import ERROR_RATE
...
def kline_to_df(ticker_st,start_date,end_date)-> pd.DataFrame:
    '''kline = pd.DataFrame(
        arr,#only input var
    ...
    try:
        ...
        #print('company name')
        #print(tikp)
        kline = yf.download(ticker_st, start=start_date, end=end_date)
    except:
...
def search_func(df, only_last=False):
